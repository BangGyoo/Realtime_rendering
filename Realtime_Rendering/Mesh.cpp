...
Mesh::Mesh(const char *filename, ShaderProgram * sh)
{
	...
	shader->addUniform("Light.Ia");
	shader->addUniform("Light.Id");
	shader->addUniform("Light.Is");
	...
	shader->addUniform("NormalMatrix"); //Refer next slide : mat4
...
void Mesh::draw(glm::mat4 ModelViewMatrix, glm::mat4 NormalMatrix, glm::mat4 inverseModelView, glm::mat4 mvp) {
	


	shader->use();
	glUniform4fv(shader->uniform("Light.Position"), 1, glm::value_ptr(global::lightPos)); //for LightPos
	glUniform3fv(shader->uniform("Light.Ia"), 1, glm::value_ptr(global::Ia)); //for Ia
	glUniform3fv(shader->uniform("Light.Id"), 1, glm::value_ptr(global::Id)); //for Id
	glUniform3fv(shader->uniform("Light.Is"), 1, glm::value_ptr(global::Is)); //for Is

	glUniformMatrix4fv(shader->uniform("ModelViewMatrix"), 1, GL_FALSE, glm::value_ptr(ModelViewMatrix)); //modelView
	glUniformMatrix3fv(shader->uniform("NormalMatrix"), 1, GL_FALSE, glm::value_ptr(NormalMatrix)); //normalMatrix
	glUniformMatrix4fv(shader->uniform("mvp"), 1, GL_FALSE, glm::value_ptr(mvp)); //mvp //uniform 변수(mvp) 값을 vertex 쉐이더로 보내줌
	...
	for (int i = 0; i < meshEntries.size(); ++i) {
		...
		glUniform3fv(shader->uniform("Material.Kd"), 1, glm::value_ptr(diffuse));
		glUniform3fv(shader->uniform("Material.Ka"), 1, glm::value_ptr(ambient));
		glUniform3fv(shader->uniform("Material.Ks"), 1, glm::value_ptr(specular));
		glUniform1f(shader->uniform("Material.Shiness"), shiness);
